...
def resend_token_user(accountid: str):
    ...
    if not user_info:
        ...
        api_reply = response_processing(answer_code)
        return jsonify(api_reply), 403
    if authorization_info and (authorization_info.username == user_info.email) and (
    ...
    answer_code = '07'
    return jsonify(response_processing(answer_code)), 403
...
def delete_user(accountid: str):
    ...
    if get_user_info(accountid):
        answer_code = '09'
        answer_code_http = 403
    return jsonify(response_processing(answer_code)), answer_code_http
...
def list_cards(accountid: str):
    ...
    return jsonify(response_processing(answer_code, api_info)), answer_code_http
